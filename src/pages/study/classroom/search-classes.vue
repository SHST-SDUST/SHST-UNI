<template>
    <view>
        <layout>
            <view class="y-center a-flex-space-around">
                <view class="select-con x-center">
                    <picker :value="floorIndex" :range="floorGroup" @change="floorSelectChange">
                ...
                <view class="select-con x-center">
                    <picker
                        ...
                        :range="classroomGroup"
                        @change="classroomSelectChange"
                    >
        ...
        <layout v-if="show" :title="classroomGroup[classroomIndex]" :top-space="true">
            ...
            <view v-for="(item, itemIndex) in 5" :key="item">
                <view class="a-flex">
                    <view v-for="(inner, innerIndex) in 7" :key="inner" class="a-full">
                        <view
                            v-if="table[innerIndex] && table[innerIndex][itemIndex]"
                            class="timetable-hide"
                        ...
                        >
                            ...
                            <view>
                                {{ table[innerIndex][itemIndex].date_start.replace(/\d{4}-/, "") }}
                            </view>
                        </view>
    ...
    methods: {
        ...
        loadClassroom: function () {
            uni.$app.throttle(500, async () => {
                const data = { classroom: this.classroomGroup[this.classroomIndex] };
                ...
                res.data.data.forEach(v => {
                    ...
                    v.background =
                        uni.$app.data.colorList[uniqueNum % uni.$app.data.colorList.length];
