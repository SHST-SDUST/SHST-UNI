        ...
        <layout title="图书信息">
            <view class="y-center">
                <view class="img-cotainer a-lmr a-flex-none">
                    <image class="img-cotainer x-center y-center" :src="book.img"></image>
                </view>
                <view class="tips-con">
...
export default {
    ...
    onLoad: async function (option) {
        ...
        uni.$app.onload(async () => {
            ...
            const detail = [];
            regMatch(/<tr><td>([\S]*?)<\/?td><\/tr>/g, res.data.info).forEach(value =>
                detail.push(value)
            );
            const liBookInfo = regMatch(
                /<ul data-role="listview">[\s\S]*?<\/ul>/g,
                res.data.info
            )[0];
            const stroage = regMatch(/<p.*?>(.*?)<\/p>/g, liBookInfo);
